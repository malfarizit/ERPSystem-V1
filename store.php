        ...
        <input type="hidden" id="old_Gpi_number" name="old_Gpi_number" value="<?php echo $ED['Gpi_number'] ; ?>" />
            ...
            <tr> 	
                    ...
                    <tr valign="baseline">
                      ...
                      <td>:</td>
                      <td><select name="Part_Number" id="Part_Number">
							<?php
                            if($bs){
                            foreach($bs as $n => $v){
                                $sel = '';
                                if($v['Part_Number']==$ED['Part_Number']){$sel ='selected';}
                                echo '<option value="'.$v['Part_Number'].'" '.$sel.'>'.$v['Part_Number'].'</option>';	
                                }
                            }
                            ?> 
        					</select> </td>
        ...
        <td colspan="3" align="center" ><b>Modifikasi</b></td>
      	...
      	if($sh){
            ...
            <td width="53" align="center" bgcolor="#CCCCCC"><a href="javascript:if (confirm('Anda Yakin Untuk Menghapus Data?'))
             { 	window.location.href='store.php?action=hapus&kd=<?php echo $v['id']; ?>' }
